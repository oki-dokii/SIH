...
def test_upload_dpr():
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"Failed to upload DPR: {response.text}")
            sys.exit(1)
            
        dpr_data = response.json()
        dpr_id = dpr_data["id"]
        print(f"DPR uploaded with ID: {dpr_id}")
        
        # 3. Verify DPR is associated with project
        print(f"Verifying DPR {dpr_id} is in project {project_id}...")
        ...
        if found:
            print("Verification successful: DPR found in project.")
        else:
            print("Verification failed: DPR not found in project list.")
            sys.exit(1)
