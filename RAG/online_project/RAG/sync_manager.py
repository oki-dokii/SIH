...
class SyncManager:
        ...
        for project in dirty_projects:
            try:
                if project['remote_id']:
                    # Update existing cloud project - send only common columns
                    ...
                    # Create new cloud project - send only common columns
        ...
        try:
            ...
            for cloud_file in cloud_files:
                ...
                if not existing:
                    ...
                    if local_project:
                        # Create new local file record from cloud
                        # Only sync common metadata: id, project_id, filename, original_filename
                        # filepath and upload_ts are environment-specific, not synced
                        cursor.execute("""
                            INSERT INTO pdfs 
                            (filename, original_filename, project_id, 
                             remote_id, dirty, processing_status)
                            VALUES (?, ?, ?, ?, 0, 'cloud_only')
                        """, (
                            cloud_file['filename'],
    ...
    def sync_up_deletions(self) -> int:
        """
        Push local deletions to cloud backend.
        
        Returns:
            Number of deletions synced
        """
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        
        # Get dirty deleted projects (marked for deletion locally)
        cursor.execute("""
            SELECT * FROM projects 
            WHERE dirty = 1 AND deleted = 1 AND remote_id IS NOT NULL
        """)
        
        deleted_projects = [dict(row) for row in cursor.fetchall()]
        synced_count = 0
        
        for project in deleted_projects:
            try:
                # Send deletion to cloud
                response = requests.post(
                    f"{self.cloud_url}/projects/{project['remote_id']}/delete",
                    timeout=10
                )
                
                if response.status_code == 200:
                    # Mark as synced (no longer dirty)
                    cursor.execute("""
                        UPDATE projects 
                        SET dirty = 0, last_synced_ts = datetime('now')
                        WHERE id = ?
                ...
                print(f"❌ Failed to sync deletion of project {project['id']}: {e}")
        
        conn.commit()
        conn.close()
        
        if synced_count > 0:
            print(f"🗑️ Synced up {synced_count} deletions")
        
        return synced_count
    
    def sync_down_deletions(self) -> int:
        """
        Pull deletions from cloud and mark local records as deleted.
        ...
        try:
            # Get deleted projects from cloud since last sync
            response = requests.get(
                f"{self.cloud_url}/projects/deleted",
                params={"since": last_sync},
                timeout=10
            )
            ...
            deleted_projects = response.json().get('deleted_projects', [])
            ...
            for cloud_project in deleted_projects:
                # Find local project by remote_id
                cursor.execute("""
                    SELECT id, deleted FROM projects WHERE remote_id = ?
                """, (cloud_project['id'],))
                
                local = cursor.fetchone()
                
                if local and not local[1]:  # Exists and not already deleted
                    # Mark as deleted (don't mark as dirty - this is from cloud)
                    cursor.execute("""
                        UPDATE projects 
                        SET deleted = 1, deleted_at = ?
                        WHERE remote_id = ?
                    """, (cloud_project['deleted_at'], cloud_project['id']))
                    synced_count += 1
            ...
            if synced_count > 0:
                print(f"🗑️ Synced down {synced_count} deletions")
            
            return synced_count
            
        except Exception as e:
            print(f"❌ Failed to sync down deletions: {e}")
            return 0
    
    def full_sync(self):
        """
        Perform a complete bidirectional sync including deletions.
        ...
        self.sync_up_deletions()  # NEW: Push deletions
        
        # Sync down (pull cloud updates)
        self.sync_down_projects()
        self.sync_down_deletions()  # NEW: Pull deletions
